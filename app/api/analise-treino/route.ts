...
export async function POST(req: NextRequest) {
  try {
    const { prompt, modo, usuarioId } = await req.json()
    ...
    // Planos nutricionais (4096 tokens) têm limite mais restrito que análises simples
    const endpoint = isPlano ? 'plano' : 'analise'
    const rl = await checkRateLimit(usuarioId ?? null, endpoint)
