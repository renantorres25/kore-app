  ...
  return (
    <main className="min-h-[100dvh] text-white md:flex" style={{ background: '#0d1117' }}>
      ...
      <div className="max-w-md mx-auto px-4 pb-28 md:max-w-4xl md:px-8 md:max-w-2xl md:px-8" style={{ paddingTop: 'max(3rem, calc(env(safe-area-inset-top) + 1.5rem))' }}>
        ...
        <div className="rounded-2xl border border-white/[0.11] p-4 mb-5" style={{ background: '#131b2e' }}>
        ...
        {Object.keys(grupos).length === 0 ? (
          <div className="rounded-2xl border border-white/[0.11] p-10 text-center" style={{ background: '#131b2e' }}>
            ...
            {Object.entries(grupos).map(([data, ags]) => (
              <div key={data}>
                ...
                <div className="space-y-2">
                  {ags.map(ag => (
                    <div key={ag.id} className={`rounded-2xl border overflow-hidden ${ag.status === 'realizado' ? 'border-white/[0.14] opacity-60' : 'border-white/[0.14]'}`} style={{ background: '#131b2e' }}>
