...
const perfis = [
  {
    tipo: 'cliente',
    emoji: '🏃',
    titulo: 'Cliente / Atleta',
    descricao: 'Quero treinar, acompanhar minha evolução e me conectar com profissionais.',
  },
  {
    tipo: 'personal',
    emoji: '🏋️',
    titulo: 'Personal Trainer',
    descricao: 'Quero montar treinos, acompanhar meus alunos e ver a evolução deles.',
  },
  {
    tipo: 'nutricionista',
    emoji: '🥗',
    titulo: 'Nutricionista',
    descricao: 'Quero prescrever dietas com base nos dados reais dos meus pacientes.',
  },
]
...
export default function Onboarding() {
  ...
  async function handleConfirmar() {
    ...
    if (!selecionado) {
      setErro('Selecione seu perfil para continuar.')
      return
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="mb-8">
          <label className="text-zinc-400 text-xs uppercase tracking-widest mb-3 block">
            Quem é você na plataforma?
          </label>
          <div className="space-y-3">
            {perfis.map((p) => (
              <button
                key={p.tipo}
                onClick={() => setSelecionado(p.tipo)}
              ...
              >
                ...
                <p className={`text-xs leading-relaxed ${
                  selecionado === p.tipo ? 'text-zinc-600' : 'text-zinc-500'
                }`}>
                  {p.descricao}
                </p>
              </button>
        ...
        <button
          ...
          disabled={!nome.trim() || !selecionado || carregando}
