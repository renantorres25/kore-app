...
export default function BemEstar() {
  ...
  function getMediaGeral() { const vals = Object.values(valores).filter(v => v > 0); if (!vals.length) return null; return Math.round(vals.reduce((a, b) => a + b, 0) / vals.length) }
  ...
  async function handleSalvar() {
    ...
    setSalvando(true)
    setErro('')
    const payload = { usuario_id: userId, data: getTodayBR(), energia: valores.energia, humor: valores.humor, dor_muscular: valores.dor_muscular, qualidade_sono: valores.qualidade_sono, notas: notas.trim() || null }
    let error
    if (jaRegistrou && registroId) { const res = await supabase.from('bem_estar').update(payload).eq('id', registroId); error = res.error }
    else { const res = await supabase.from('bem_estar').insert(payload); error = res.error }
  ...
  if (carregando) return (<main className="min-h-screen bg-black flex items-center justify-center"><div className="w-8 h-8 border-2 border-white border-t-transparent rounded-full animate-spin" /></main>)

  if (sucesso) return (
    <main className="min-h-screen bg-black flex items-center justify-center">
      <div className="flex flex-col items-center gap-4"><div className="text-6xl">✅</div><p className="text-white font-bold text-lg">Registrado!</p><p className="text-zinc-500 text-sm">Voltando ao dashboard...</p></div>
    </main>
  )
    ...
    <main className="min-h-[100dvh] bg-[#0a0a0a] text-white">
      <div className="max-w-md mx-auto px-4 py-8 pb-12">
        ...
        {media && (
          <div className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800 mb-6">
            <div className="flex items-center justify-between">
              <div><p className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Seu estado geral hoje</p><p className={`text-xl font-black ${getCorMedia(media)}`}>{getLabelMedia(media)}</p></div>
              <div className="text-right"><p className={`text-4xl font-black ${getCorMedia(media)}`}>{media}</p><p className="text-zinc-600 text-xs">de 5</p></div>
            </div>
          ...
          {indicadores.map((ind) => (
            <div key={ind.id} className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800">
              ...
              <div className="flex justify-between gap-1">
                {[1,2,3,4,5].map((v) => (
                  <button key={v} onClick={() => setValor(ind.id, v)} className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-xl border transition-all active:scale-95 ${valores[ind.id] === v ? 'bg-white border-white' : 'bg-zinc-800 border-zinc-700 hover:border-zinc-500'}`}>
        ...
        <div className="mb-6">
          ...
          <textarea placeholder="Algo específico que quer registrar?" value={notas} onChange={(e) => setNotas(e.target.value)} rows={3} className="w-full bg-zinc-900 text-white placeholder-zinc-600 rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-white border border-zinc-800 resize-none" />
        </div>

        <div className="flex items-center gap-2 mb-4">
          <div className="flex gap-1">{[0,1,2,3].map((i) => <div key={i} className={`w-2 h-2 rounded-full transition-all ${i < total ? 'bg-white' : 'bg-zinc-700'}`} />)}</div>
          <p className="text-zinc-500 text-xs">{total} de 4 preenchidos</p>
        </div>

        {erro && <p className="text-red-400 text-sm text-center mb-4">{erro}</p>}

        <button onClick={handleSalvar} disabled={salvando || total < 4} className="w-full bg-white text-black font-bold py-4 rounded-2xl hover:bg-zinc-100 active:scale-95 transition-all disabled:opacity-30 text-sm tracking-widest uppercase">
