    ...
    <main className="min-h-screen text-white overflow-x-hidden" style={{ background: '#06070f' }}>
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-5 lg:px-10 py-4"
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-emerald-400 border border-emerald-500/40 bg-emerald-500/10 rounded-full px-2 py-0.5 uppercase tracking-widest font-bold">Beta</span>
        </div>
      ...
      <section className="relative min-h-screen flex items-center pt-20 pb-10 px-5 overflow-hidden">
        ...
        {/* Fundo: foto de academia dramática */}
        ...
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          ...
          <div className="orb-c absolute top-[50%] left-[40%] w-[400px] h-[400px] rounded-full opacity-[0.06] blur-[80px]"
          ...
          <div>
            <div className={`mb-6 transition-all duration-700 ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
              <span className="inline-flex items-center gap-2 border border-emerald-500/30 bg-emerald-500/[0.08] rounded-full px-4 py-1.5">
                ...
                <span className="text-emerald-400 text-[11px] uppercase tracking-[0.25em] font-bold">Performance integrada · Brasil</span>
              </span>
            </div>

            <h1 className={`font-black tracking-[-0.06em] leading-none mb-6 transition-all duration-700 delay-100 ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
              <span className="block text-[5.5rem] sm:text-[6.5rem]" style={{ textShadow: '0 0 100px rgba(16,185,129,0.2)' }}>KORE</span>
              <span className="block text-2xl sm:text-3xl text-zinc-200 font-bold tracking-normal mt-2 leading-snug">O sistema operacional<br />do seu corpo.</span>
            </h1>
            ...
            <p className={`text-zinc-300 text-base leading-relaxed mb-8 max-w-sm transition-all duration-700 delay-[200ms] ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
              Atleta, personal trainer e nutricionista na mesma plataforma. A IA analisa sono, HRV e bem-estar para gerar a <strong className="text-white">decisão exata</strong> do que fazer a cada dia.
            </p>
            ...
            <div className={`mt-8 flex items-center gap-4 transition-all duration-700 delay-[360ms] ${vis ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
              ...
              <div>
                ...
                <p className="text-zinc-400 text-xs">Atletas e profissionais ativos</p>
              </div>
            </div>
          </div>
      ...
      {/* STATS                                                             */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section ref={stats.ref} className="relative px-5 py-20 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-px rounded-3xl overflow-hidden border border-white/[0.08]" style={{ background: 'rgba(255,255,255,0.04)' }}>
            {[
              ...
              { n: n2, sfx: ' em 1', label: 'perfis diferentes conectados na mesma plataforma em tempo real', sub: 'Atleta · Personal · Nutricionista' },
              { n: n3, sfx: ' dim.', label: 'dimensões monitoradas diariamente para gerar a decisão do dia', sub: 'Sono · Treino · Nutrição · Bem-estar' },
            ].map((s, i) => (
              <div key={i} className="p-8 flex flex-col gap-3" style={{ background: 'rgba(12,14,24,0.92)', backdropFilter: 'blur(8px)' }}>
                <p className="text-[4rem] font-black tracking-tight leading-none">
                  {s.n}<span className="text-emerald-400 text-[2.6rem]">{s.sfx}</span>
                </p>
                <div>
                  <p className="text-zinc-300 text-sm leading-relaxed">{s.label}</p>
                  <p className="text-zinc-500 text-xs mt-1.5 font-medium">{s.sub}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section ref={perfis.ref} className="relative px-5 py-16 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-zinc-400 text-base mt-4 max-w-md mx-auto">Cada perfil tem acesso ao que importa — sem informação de menos, sem sobrecarga.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {([
              ...
              { sigla: 'PT', cor: 'blue' as const,    role: 'Personal Trainer', headline: 'Monitore todos em tempo real.', desc: 'Veja o sono, recuperação e bem-estar de cada aluno antes de prescrever qualquer coisa.', items: ['Dashboard de todos os alunos', 'Alertas quando aluno precisa de atenção', 'Agenda e histórico integrados'] },
              { sigla: 'NU', cor: 'green' as const,   role: 'Nutricionista', headline: 'Prescreva com dados reais.', desc: 'Crie planos alimentares para o momento atual do paciente — não para um perfil genérico.', items: ['Vê sono, treino e energia do paciente', 'Plano alimentar com IA ou manual', 'Agenda de consultas integrada'] },
            ] as const).map(p => {
              const C = {
              ...
              return (
                <div key={p.role} className={`rounded-2xl p-6 border ${C.border} flex flex-col`}
                  style={{ background: 'rgba(12,14,24,0.9)', backdropFilter: 'blur(8px)' }}>
      ...
      <section className="px-5 py-10 max-w-5xl mx-auto space-y-5">
        <div className="text-center mb-14">
          ...
          <h2 className="text-4xl sm:text-5xl font-black uppercase tracking-tight">
            Inteligência real.<br /><span className="text-zinc-400">Não só números.</span>
          </h2>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-8 lg:p-10 flex flex-col justify-center">
              <span className="inline-flex items-center gap-2 border border-emerald-500/30 bg-emerald-500/10 rounded-full px-3 py-1.5 mb-5 w-fit">
                ...
                <span className="text-emerald-400 text-[10px] uppercase tracking-widest font-bold">IA proprietária</span>
              </span>
              <h3 className="text-2xl sm:text-3xl font-black mb-4 uppercase tracking-tight">Decisão do dia</h3>
              <p className="text-zinc-300 text-sm leading-relaxed mb-4">Toda manhã, nossa IA cruza dados de sono, HRV, bem-estar e histórico de treino para gerar <strong className="text-white">uma recomendação concreta</strong> — treinar forte, moderado ou descansar.</p>
              <p className="text-zinc-400 text-sm border-l-2 border-emerald-500/40 pl-4">Não é um dashboard genérico. É uma decisão.</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            ...
            <div className="p-8 lg:p-10 flex flex-col justify-center order-1 md:order-2">
              <span className="inline-flex items-center gap-2 border border-orange-500/30 bg-orange-500/10 rounded-full px-3 py-1.5 mb-5 w-fit">
                <span className="text-orange-400 text-[10px] uppercase tracking-widest font-bold">📈 Evolução real</span>
              </span>
              <h3 className="text-2xl sm:text-3xl font-black mb-4 uppercase tracking-tight">Veja onde estava.<br />Veja onde está.</h3>
              <p className="text-zinc-300 text-sm leading-relaxed mb-4">O histórico de treinos, recuperação e hábitos se acumula ao longo do tempo. Cada quadrado é um dia. A cor, a intensidade. <strong className="text-white">Padrões que você não veria sozinho.</strong></p>
              <p className="text-zinc-400 text-sm border-l-2 border-orange-500/40 pl-4">Overtraining, subrecuperação e consistência — visíveis com um olhar.</p>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2">
            <div className="p-8 lg:p-10 flex flex-col justify-center">
              <span className="inline-flex items-center gap-2 border border-violet-500/30 bg-violet-500/10 rounded-full px-3 py-1.5 mb-5 w-fit">
                <span className="text-violet-400 text-[10px] uppercase tracking-widest font-bold">😴 Score de sono</span>
              </span>
              <h3 className="text-2xl sm:text-3xl font-black mb-4 uppercase tracking-tight">Recuperação não é intuição.</h3>
              <p className="text-zinc-300 text-sm leading-relaxed mb-4">Score calculado com duração, HRV, sono profundo e REM. A IA usa esses dados para calibrar <strong className="text-white">toda a prescrição do dia</strong> — do treino à dieta.</p>
              <p className="text-zinc-400 text-sm border-l-2 border-violet-500/40 pl-4">Um dia mal dormido muda tudo. O KORE percebe antes de você.</p>
            </div>
            <div className="p-8 flex flex-col items-center justify-center border-t md:border-t-0 md:border-l border-white/[0.06]" style={{ background: '#0a0c16' }}>
              <p className="text-zinc-400 text-xs uppercase tracking-widest mb-5 self-start font-medium">Score de recuperação · últimos 7 dias</p>
      ...
      <section ref={testi.ref} className="relative px-5 py-20 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl sm:text-5xl font-black uppercase tracking-tight">Resultados reais.</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {DEPOIMENTOS.map((d, i) => {
              const C = { emerald: 'bg-emerald-500', green: 'bg-green-500', blue: 'bg-blue-500' }[d.cor]
              return (
                <div key={i}
                  className={`rounded-2xl p-6 border border-white/[0.09] flex flex-col gap-4 transition-all duration-700 ${testi.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}
                  style={{ background: 'rgba(12,14,24,0.93)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', transitionDelay: `${i * 120}ms` }}>
                  <div className="flex gap-0.5 mb-1">
                    {[...Array(5)].map((_, s) => <span key={s} className="text-yellow-400 text-sm">★</span>)}
                  </div>
                  <p className="text-zinc-200 text-sm leading-relaxed flex-1">"{d.quote}"</p>
                  <div className="flex items-center gap-3 pt-4 border-t border-white/[0.06]">
                    <div className={`w-9 h-9 rounded-xl ${C} flex items-center justify-center text-xs font-black text-white`}>{d.avatar}</div>
                    <div>
                      <p className="text-white text-sm font-bold">{d.nome}</p>
                      <p className="text-zinc-400 text-xs">{d.cargo}</p>
                    </div>
                  </div>
                </div>
              )
      ...
      <section ref={como.ref} className="relative px-5 py-16 overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="text-4xl sm:text-5xl font-black uppercase tracking-tight">
              Simples de começar.<br /><span className="text-zinc-400">Poderoso na prática.</span>
            </h2>
          </div>
          <div className="space-y-4 max-w-2xl mx-auto">
            {[
              { n: '01', title: 'Crie sua conta em 60 segundos', desc: 'Escolha seu perfil — atleta, personal trainer ou nutricionista. Sem cartão de crédito. Sem compromisso.', cor: 'text-emerald-400 border-emerald-500/35 bg-emerald-500/10' },
              { n: '02', title: 'Conecte seu time de profissionais', desc: 'O profissional envia um link de convite. O atleta aceita. Todos passam a ver os mesmos dados em tempo real — sem WhatsApp no meio.', cor: 'text-blue-400 border-blue-500/35 bg-blue-500/10' },
              { n: '03', title: 'A IA trabalha — você executa', desc: 'Cada manhã, a IA analisa tudo e entrega a decisão do dia. Personal e nutri ajustam com base em dados reais. Atleta executa com confiança.', cor: 'text-violet-400 border-violet-500/35 bg-violet-500/10' },
            ].map((s, i) => (
              <div key={s.n}
      ...
      <section className="px-5 py-12 max-w-5xl mx-auto">
        <div className="rounded-2xl border border-white/[0.07] p-7" style={{ background: '#0c0e18' }}>
          <p className="text-zinc-400 text-xs uppercase tracking-[0.3em] text-center mb-8 font-medium">Tecnologia que sustenta o KORE</p>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              <div key={t.nome} className="text-center p-5 rounded-xl border border-white/[0.06]" style={{ background: '#0a0c16' }}>
                <div className="text-3xl mb-3">{t.icon}</div>
                <p className="text-white text-sm font-bold">{t.nome}</p>
                <p className="text-zinc-400 text-xs mt-1">{t.desc}</p>
                <span className="inline-block mt-2 text-[9px] text-zinc-400 border border-white/[0.08] rounded-full px-2.5 py-0.5 font-medium">{t.badge}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative rounded-3xl border border-emerald-500/25 p-10 sm:p-16 text-center overflow-hidden">
          ...
          <div className="relative">
            <span className="inline-flex items-center gap-2 border border-emerald-500/30 bg-emerald-500/10 rounded-full px-4 py-2 mb-6">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 pulse-ring"/>
              <span className="text-emerald-400 text-[11px] uppercase tracking-[0.25em] font-bold">Gratuito para começar</span>
            </span>
      ...
      <footer className="border-t border-white/[0.06] px-5 py-8">
        <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-zinc-500 text-xs">Performance integrada</span>
          </div>
          <div className="text-center sm:text-right">
            <p className="text-zinc-500 text-xs">© 2026 KORE — Todos os direitos reservados</p>
            <p className="text-zinc-600 text-[10px] mt-0.5">Feito no Brasil 🇧🇷 para atletas e profissionais</p>
          </div>
        </div>
      </footer>
    </main>
