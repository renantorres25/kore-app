...
const MOD_CONFIG: Record<string, { icon: string; cor: string; corText: string; corBorder: string }> = {
  musculacao: { icon: '🏋️', cor: 'bg-emerald-500/10', corText: 'text-emerald-400', corBorder: 'border-emerald-500/20' },
  corrida:    { icon: '🏃', cor: 'bg-blue-500/10',    corText: 'text-blue-400',    corBorder: 'border-blue-500/20'    },
  bike:       { icon: '🚴', cor: 'bg-orange-500/10',  corText: 'text-orange-400',  corBorder: 'border-orange-500/20'  },
  natacao:    { icon: '🏊', cor: 'bg-cyan-500/10',    corText: 'text-cyan-400',    corBorder: 'border-cyan-500/20'    },
  crossfit:   { icon: '⚡', cor: 'bg-yellow-500/10',  corText: 'text-yellow-400',  corBorder: 'border-yellow-500/20'  },
  outro:      { icon: '🎯', cor: 'bg-purple-500/10',  corText: 'text-purple-400',  corBorder: 'border-purple-500/20'  },
  // planos musculação
  A:          { icon: '💪', cor: 'bg-emerald-500/10', corText: 'text-emerald-400', corBorder: 'border-emerald-500/20' },
  B:          { icon: '💪', cor: 'bg-blue-500/10',    corText: 'text-blue-400',    corBorder: 'border-blue-500/20'    },
  C:          { icon: '💪', cor: 'bg-orange-500/10',  corText: 'text-orange-400',  corBorder: 'border-orange-500/20'  },
}
...
export default function Evolucao() {
  ...
  async function carregar() {
    ...
    // Monta lista unificada de atividades
    ...
    treinosData?.forEach((t: any) => {
      lista.push({
        data: t.data,
        tipo: 'musculacao',
        nome: t.nome ?? `Treino ${t.plano}`,
        detalhe: `Plano ${t.plano}`,
        calorias: t.calorias_estimadas ?? null,
      })
    })
    ...
    atividadesData?.forEach((a: any) => {
      ...
      lista.push({
        data: a.data,
        tipo: a.modalidade,
        nome: modLabel[a.modalidade] ?? 'Atividade',
        detalhe,
    ...
    // Streak — considera todas as atividades
    ...
    // Volume por exercício (musculação)
    let seriesData: any[] = []
...
ÚLTIMOS 30 DIAS:
- Total de atividades: ${totalAtividades} (musculação + corrida + natação + outros esportes)
...
Análise em 3 partes (máx 100 palavras, sem markdown): Consistência e tendência, Ponto mais forte, Foco para os próximos 7 dias com ação concreta`
  ...
  // Agrupa atividades por data para o histórico
        ...
        {!temDados ? (
          ...
          <>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[
                { val: streak, label: 'Streak', icon: '🔥', cor: streak >= 7 ? 'text-orange-400' : 'text-white' },
                { val: totalAtividades, label: 'Atividades', icon: '💪', cor: 'text-emerald-400' },
                { val: mediaScore ?? '—', label: 'Recup.', icon: '⚡', cor: mediaScore && mediaScore >= 70 ? 'text-emerald-400' : mediaScore ? 'text-yellow-400' : 'text-zinc-500' },
                { val: totalCalorias > 0 ? `${Math.round(totalCalorias/1000)}k` : '—', label: 'kcal', icon: '🔥', cor: 'text-orange-400' },
              ].map((s, i) => (
              ...
              <div className="px-5 py-4">
                {!analise.gerado && !analise.carregando && (
                  <div className="flex flex-col gap-3">
                    <p className="text-zinc-500 text-sm">A IA analisa todas suas atividades — musculação, corrida, natação e mais — junto com recuperação e consistência.</p>
                    <button onClick={gerarAnaliseIA} className="w-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 font-bold py-3 rounded-xl text-sm active:scale-95 hover:bg-emerald-500/20 transition-all">
