...
function getScoreRecuperacao(sono: DadosSono, bemEstar: DadosBemEstar | null) {
  ...
  if (sono.hrv) {
    const pontos = Math.min(25, (sono.hrv / 100) * 25)
    score += pontos; total += 25
  }
  if (bemEstar) {
    const mediaSubjetiva = (bemEstar.energia + bemEstar.humor + (6 - bemEstar.dor_muscular)) / 3
    score += (mediaSubjetiva / 5) * 25; total += 25
  }
...
export default function Sono() {
  ...
  const [sono, setSono] = useState<DadosSono>({
    ...
    <main className="min-h-[100dvh] bg-[#0a0a0a] text-white">
      <div className="max-w-md mx-auto px-4 py-8 pb-12">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-zinc-500 text-xs capitalize">{new Date().toLocaleDateString('pt-BR', { weekday: 'long', day: 'numeric', month: 'long' })}</p>
            </div>
          </div>
          {temDadosSono && (
            <button
              onClick={() => setEditando(!editando)}
              className="text-[10px] text-zinc-400 border border-zinc-700 rounded-lg px-3 py-1.5 hover:border-white hover:text-white transition-all uppercase tracking-wider active:scale-95"
            >
        ...
        {temDadosSono && score && !editando && (
          <div className="relative bg-gradient-to-br from-zinc-800 via-zinc-900 to-black rounded-2xl p-6 border border-zinc-700/40 mb-4 overflow-hidden">
            ...
            <div className="h-2 bg-zinc-800 rounded-full">
              <div
                className={`h-2 rounded-full transition-all duration-700 ${corScore?.bg}`}
                style={{ width: `${score}%` }}
              />
            </div>
          </div>
        ...
        {temDadosSono && !editando && (
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 mb-4 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-800">
              <p className="text-zinc-400 text-[10px] uppercase tracking-widest">Dados da noite</p>
            </div>
            <div className="grid grid-cols-2 divide-x divide-y divide-zinc-800">
              ...
              <div className="p-4">
                <p className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Qualidade</p>
                <p className="text-white text-xl font-black">{sono.qualidade ? `${sono.qualidade}/5` : '—'}</p>
                <p className="text-zinc-600 text-[10px] mt-0.5">Subjetiva</p>
              </div>
              <div className="p-4">
              ...
              <div className="p-4 col-span-2">
                <p className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">SpO2</p>
                <p className="text-white text-xl font-black">{sono.spo2 ? `${sono.spo2}%` : '—'}</p>
                <p className="text-zinc-600 text-[10px] mt-0.5">{sono.spo2 ? (sono.spo2 >= 95 ? '✓ Normal' : '⚠ Abaixo do ideal') : 'Wearable'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {bemEstar && !editando && (
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 mb-4 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-800">
              <p className="text-zinc-400 text-[10px] uppercase tracking-widest">Cruzamento com bem-estar</p>
            </div>
            ...
            <div className="p-5">
              {analisando && (
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-zinc-400 text-sm">Analisando seus dados...</p>
                </div>
              )}
              ...
              {!analiseIA && !analisando && (
                <div className="flex flex-col items-center py-4 gap-3">
                  ...
                  <button
                    onClick={gerarAnaliseIA}
                    className="bg-white text-black font-bold px-6 py-3 rounded-xl text-sm hover:bg-zinc-100 active:scale-95 transition-all"
                  >
                    ✦ Gerar análise
                  </button>
                </div>
        ...
        {historico.length > 0 && !editando && (
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 mb-4 overflow-hidden">
            <div className="px-5 py-4 border-b border-zinc-800">
              <p className="text-zinc-400 text-[10px] uppercase tracking-widest">Últimos 7 dias</p>
            </div>
            <div className="p-5">
              ...
              <div className="flex gap-3 mt-3">
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400" /><span className="text-zinc-600 text-[10px]">Ótimo</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-yellow-400" /><span className="text-zinc-600 text-[10px]">Regular</span></div>
                <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-400" /><span className="text-zinc-600 text-[10px]">Ruim</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {editando && (
          <div className="space-y-4">
            ...
            <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-5">
              ...
              <p className="text-zinc-600 text-xs mb-4">Opcional — preencha se tiver aparelho conectado</p>
          ...
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <p className="text-zinc-500 text-xs">Apple Watch, Garmin, Whoop, Polar e mais</p>
              </div>
            </div>
            <button className="w-full border border-zinc-700 text-zinc-400 font-semibold py-3 rounded-xl text-sm hover:border-white hover:text-white active:scale-95 transition-all uppercase tracking-wider">
              Em breve
            </button>
          </div>
        )}
