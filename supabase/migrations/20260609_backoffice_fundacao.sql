...
-- ----------------------------------------------------------------------------
-- ADITIVO e IDEMPOTENTE: cria apenas tabelas novas e referencia perfis(id).
-- Pode ser rodado mais de uma vez com segurança (tabelas: if not exists;
-- políticas: drop if exists + create).
-- NÃO altera nenhuma tabela existente.
--
-- ⚠️  ANTES DE APLICAR: aplicar em staging/branch ou após BACKUP (plano free
--     pode não ter backup automático — mas o script é aditivo).
--     Após aplicar, cadastrar o primeiro super_admin (ver final do arquivo).
-- ============================================================================
...
drop policy if exists eventos_select_admin on eventos;
create policy eventos_select_admin on eventos for select using (is_admin());

-- ── 3. Assinaturas e pagamentos (provedor-agnóstico) ────────────────────────
...
drop policy if exists assinaturas_admin_write on assinaturas;
...
drop policy if exists pagamentos_admin_write on pagamentos;
...
-- Sem políticas de UPDATE/DELETE: o log é imutável por design.
...
drop policy if exists consent_insert_own on consentimentos;
create policy consent_insert_own on consentimentos for insert to authenticated with check (user_id = auth.uid());
drop policy if exists consent_select on consentimentos;
create policy consent_select     on consentimentos for select using (user_id = auth.uid() or is_admin());

-- ============================================================================
-- PÓS-APLICAÇÃO — cadastrar o primeiro super_admin (rodar manualmente):
--   select id, nome, email from perfis where email = 'seu@email.com';
--   insert into admin_users (user_id, papel) values ('<UUID-do-seu-perfil>', 'super_admin');
-- ============================================================================
